...
module.exports = {
  ...
  replicateMonth(srcKey, dstKey) {
    ...
    const cloned = src.map(t => ({ ...t, id: crypto.randomUUID(), paid: t.type === 'expense' ? false : undefined }));
    db.monthly[dstKey] = cloned;
    writeAll(db);
  ...
  cancelRecurrence(fromKey, recurringId) {
    const db = readAll();
    for (const monthKey of Object.keys(db.monthly)) {
      if (monthKey > fromKey) {
        db.monthly[monthKey] = (db.monthly[monthKey] || []).filter(
          t => t.recurringId !== recurringId
        );
      }
    }
    writeAll(db);
    return db.monthly[fromKey] || [];
  },
